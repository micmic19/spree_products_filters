Spree::ProductsHelper.class_eval do
  ...
  def link_to_sort(key)
    ...
    # params.permit!
    link_to Spree.t(key).html_safe, params.permit({ :filters => [:properties => [:types =>[]]] }, :types, { :filters => [:properties => []] }, :filters, { :filters => [] }, :commit, :sorting, :utf8, :page).merge(sorting: key), class: klass
  end
